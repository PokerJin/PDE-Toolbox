...
function solve_mulell_circle(xy_range,Nmmp,equ_para,bc_type,bc_para)
...
r_micro = radius / epsilon; 
c_micro = c_matrix * ones(size(Y1_micro));
dist_from_center = sqrt((Y1_micro - 0.5).^2 + (Y2_micro - 0.5).^2);
c_micro(dist_from_center <= r_micro) = c_inclusion;
