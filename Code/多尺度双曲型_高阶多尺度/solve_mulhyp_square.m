...
function solve_mulhyp_square(xyt_range,Nmmtp,equ_para,init,bc_type,bc_para)
%% ========== 用户可配置参数 ==========
...
% 几何参数
square_side = equ_para.len;      % 正方形边长

% 材料参数
c_in = equ_para.c_in;                % 夹杂物内扩散系数
c_out =equ_para.c_ma;           % 夹杂物外扩散系数
d_in = equ_para.d_in;                % 夹杂物内密度
d_out = equ_para.d_ma;               % 夹杂物外密度
...
square_side_cell = square_side/epsilon; 
half_side = square_side_cell/2;
in_square = (abs(y1-0.5) <= half_side) & (abs(y2-0.5) <= half_side);
c_cell = c_out * ones(n_micro, n_micro);
c_cell(in_square) = c_in;
d_cell = d_out * ones(n_micro, n_micro);
d_cell(in_square) = d_in;
